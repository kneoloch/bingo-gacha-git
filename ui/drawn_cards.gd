...
@export var card_offset_x: float = 60.0
...
func _process(delta):
	...
	for i in range(get_child_count()):
		var c: TextureButton = get_child(i)
		var val: float = sin(i + (time * time_multiplier))
		#print("Child %d, sin(%d) = %f" % [i, i, val])
		c.position.y += val * sine_offset_mult

func draw_starter_cards(from_pos: Vector2, number: int, drawn_card_collection: Array) -> void:
	drawn = true
	...
	for i in range(number):
		var instance: CardItem = card_scene.instantiate()
		instance.name = "Card %d" % (i + 1)
		add_child(instance)
		instance.global_position = from_pos
		instance.generate_card(drawn_card_collection[i])
		instance.add_to_group("card_drawn")
		Gacha.unclaimed_hand.append(instance)
...
func animate_cards() -> void:
	...
	for i in range(get_child_count()):
		var _c: TextureButton = get_child(i)
